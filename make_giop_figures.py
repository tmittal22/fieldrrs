...
#: What each GIOP figure shows, by METHOD not by result -- this table is shared across
#: every station, so a station-specific number baked in here would be wrong everywhere
#: except the one station it was written against. That mistake was made once already
#: (a hand-written GIOP/README.md existed for LOC1 only, quoted LOC1's own numbers, and
#: had already gone stale against later corrections in the same file -- referencing a
#: figure by its old pre-rename name, and a_dg/b_bp values later withdrawn). This table
#: plus write_giop_index() below replaces it with something generated fresh every run.
GIOP_FIG_INDEX = [
    ...
    ("giop1_band_choice.png", "Band choice",
     "6 bands vs 10/20 nm vs hyperspectral. GIOP is not a six-band algorithm -- it "
     "solves at whatever grid it is given."),
    ("giop2_seed_chain.png", "The seed chain",
     "Everything prescribed before any fitting happens: OC4 -> the Bricaud a*_phi "
     "SHAPE (not its amplitude), eta and S_dg from the same blue/green ratio."),
    ...
    ("giop4_conditioning.png", "Conditioning",
     "The measured shape uncertainty propagated through the inversion, both band "
     "choices -- six bands amplifies it far more than hyperspectral does."),
    ("giop5_sdg_assumption.png", "S_dg swept",
     "S_dg swept over its ordinary range, coloured by chi2 so a bad-fitting sweep arm "
     "cannot be mistaken for a real uncertainty."),
    ("giop6_all_fits.png", "Every per-scan fit",
     "GIOP on each individual angle-matched water/sky pair, constrained AND free "
     "shapes overlaid, so the misfit is not attributed to one bad mean spectrum."),
    ("giop7_covariance.png", "Per-scan covariance",
     "Every fitted parameter per scan, their cross-correlation, and which one absorbs "
     "a change in signal amplitude."),
    ("giop8_assumption_free.png", "Assumption stripping",
     "OC4 input, the a*_phi family, and freeing S_dg/eta, swept in turn -- each point "
     "coloured by its own chi2 so a rejected arm cannot be read as an uncertainty."),
    ("giop9_chi2_weighting.png", "Chi-squared weighting",
     "Does anything actually fit well? Ranks every arm by chi2 against the measured "
     "band uncertainty, maps chi2 over (S_dg, eta), and checks whether chi2 can be "
     "read as a likelihood here (it cannot -- the residual is smooth, not noise)."),
    ("giop10_final_result.png", "THE final result",
     "Per-spectrum points, their mean, and the fit of the mean spectrum, for "
     "constrained / free / maximum-freedom. The self-consistency test (does M_phi "
     "reproduce its own seed) and the headline numbers to quote."),
    ("giop11_chi2_crossplot.png", "Free vs constrained, per scan",
     "Does the mean-spectrum improvement from freeing the shapes reproduce "
     "independently in every scan, or is it an averaging artefact?"),
]


def write_giop_index(outdir, tag, mean_row, nu, findings_name):
    """GIOP/README.md -- generated fresh every run, not hand-maintained.

    Lists only figures that actually exist in outdir, and pulls every number live from
    mean_row rather than embedding it in prose, so this cannot go stale the way a
    hand-written version already did once (see GIOP_FIG_INDEX's docstring).
    """
    have = {fn for fn, _, _ in GIOP_FIG_INDEX if os.path.exists(os.path.join(outdir, fn))}
    L = ["# GIOP fits -- %s" % tag, "",
         "Generated by `python make_giop_figures.py <this location folder>`, which "
    ...
    depth = 0
    while True:
        cand = os.path.join(d, findings_name)
        if os.path.exists(cand):
            fnd_rel = os.path.relpath(cand, outdir)
            break
        if os.path.normpath(d) == os.path.normpath(script_root) or depth > 10:
            break
        d = os.path.dirname(d)
        depth += 1
    if fnd_rel:
        L.append("**Read [`%s`](%s) first** -- the physics, the caveats, and what to "
                 "quote for this station specifically." % (findings_name, fnd_rel))
    else:
        L.append("⚠ No `%s` found for this station -- the numbers below are correct "
                 "but the narrative/caveats have not been written up." % findings_name)
    L += ["", "## Figures", ""]
    for fn, title, what in GIOP_FIG_INDEX:
        if fn in have:
            L.append("### `%s` -- %s" % (fn, title)); L.append(""); L.append(what)
            L.append("")
    L += ["## Headline numbers, this station (from `giop_FINAL.csv`)", "",
         "chi2_nu is NOT comparable across stations with different measured shape "
         "uncertainty -- compare RMS misfit instead. See THEORY_GIOP_NOTE.md sect 4.",
         "", "| config | M_phi | a_dg(443) m^-1 | b_bp(443) m^-1 | chi2_nu (nu=%d) | "
         "RMS misfit |" % nu, "|---|---|---|---|---|---|"]
    for cfg in ("constrained", "free"):
        r = mean_row[cfg]
        L.append("| %s | %.3g | %.4g | %.4g | %.1f | %.1f %% |"
                 % (cfg, r["M"], r["adg"], r["bbp"], r["c2n"], r["rms"]))
    if "max free" in mean_row:
        r = mean_row["max free"]
        L.append("| max freedom | %.3g | %.4g | %.4g | %.1f | %.1f %% |"
                 % (r["M"], r["adg"], r["bbp"], r["c2n"], r["rms"]))
...
def main():
    ...
    locname = os.path.basename(os.path.dirname(loc.rstrip("/"))).split("_")[0]
    write_giop_index(a.out, os.path.basename(os.path.dirname(loc.rstrip("/"))) + "  ·  "
                     + os.path.basename(loc.rstrip("/")), mean_row, nu,
                     "%s_GIOP_FINDINGS.md" % locname)
    print("wrote %s/README.md" % a.out)
    for p in ps:
        print("wrote %s" % p)
