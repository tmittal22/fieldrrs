...
def steps(ax):
    ax.axis("off")
    ax.set_xlim(0, 10)
    ax.set_ylim(0.92, 3.18)      # cropped so the boxes hug their text
    ...
    boxes = [
        ...
        ("RECORD", "#8a7000", "#fff6d5",
         "WIND SPEED (m/s) — sets ρ,\n"
         "  the largest error in the whole\n"
         "  measurement. ρ = 0.028 is only\n"
         "  valid below ~5 m/s.\n"
        ...
        ("CHECK ON THE SPOT", GOOD, "#eaf6ea",
         ...
         "  blue = clear, green = turbid\n"
         "Replicates lie on top of\n"
         "  each other?\n"
    ...
    for i, (title, col, fc) in enumerate([(b[0], b[1], b[2]) for b in boxes]):
        body = boxes[i][3]
        x = 0.12 + i * w
        ax.add_patch(FancyBboxPatch((x, 1.00), w - 0.16, 2.08,
...
def main():
    ...
    gs = fig.add_gridspec(2, 2, height_ratios=[1.55, 0.80], width_ratios=[1.5, 1.0],
