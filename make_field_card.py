...
def page_two(fig):
    ...
    panels = [
        ...
        (gs[0, 1], "BROKEN CLOUD — the worst case", BAD, "#fdecea",
         ...
         "     uniformly overcast.\n"
         "  2. Use the SEPARATE E_d instrument (panel opposite).\n"
         "     Measuring E_d WITH the target removes the time lag,\n"
         "     which is the whole of this problem.\n"
        ...
        (gs[1, 0], "THE E_d SENSOR PATH — use it when light moves", "#8a6000", "#fff6d5",
         "A SEPARATE instrument with a WIDE (hemispherical) view,\n"
         "logging E_d at the SAME MOMENT as the radiance scans.\n"
         "In the GUI it is the FOURTH slot, LOAD ED, and loading it\n"
         "switches the physics:\n\n"
         "    E_d slot EMPTY   → E_d = π·L_panel / R_panel\n"
         "    E_d slot FILLED  → E_d measured, panel drops out\n\n"
         "Three error sources drop out at once:\n"
         "  • the panel-to-target TIME LAG  → the broken-cloud fix\n"
         "  • the PANEL REFLECTANCE (no 0.99 assumption)\n"
         "  • PANEL LEVELNESS — the collector defines the horizontal\n\n"
         "⚠ BUT IT ADDS ONE. R_rs now divides a radiance from one\n"
         "instrument by an irradiance from ANOTHER, so any offset in\n"
         "their absolute calibrations is a DIRECT MULTIPLICATIVE BIAS\n"
         "on every R_rs. A 6 % gain offset = −5.7 % on R_rs(443),\n"
         "uniform and silent.\n\n"
         "SO STILL TAKE THE PANEL SCAN. It becomes the TRANSFER\n"
         "STANDARD, not the E_d source. Load panel AND E_d and the\n"
         "GUI prints C(λ) = [π·L_p/R_p] / E_d and the verdict:\n"
         "FLAT disagreement = gain offset; STRUCTURED = stale cal,\n"
         "tilted collector or shaded panel. Do it ONCE PER DEPLOYMENT.\n\n"
         "Collector MUST BE LEVEL, same reason the panel was."),
